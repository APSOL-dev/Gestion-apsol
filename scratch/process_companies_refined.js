...
lines.forEach(line => {
  ...
  const [id_excel, nombre, provincia, pais, industria, tamanio, dias_espera] = parts.map(p => p.trim());
  ...
  if (existing) {
    ...
    updates.push({ id: targetId, ...cleanData });
  } else {
    // If it's a new company, I'll generate a UUID in the SQL
    inserts.push(cleanData);
    // Since I don't have the UUID yet, I'll use a placeholder for reasons
    // But this is tricky. For now, let's just insert the companies.
  }

  // Reasons for existing companies
  if (targetId) {
    for (let i = 7; i < parts.length; i += 2) {
      const rs = parts[i];
      const cuit = parts[i+1];
      if (rs && rs.trim() && rs !== 'null') {
        reasons.push({
          empresa_id: targetId,
          razon_social: rs.trim(),
          cuit: cuit && cuit.trim() && cuit !== 'null' ? cuit.trim() : null
        });
      }
    }
  }
...
let sql = '-- Updates\n';
updates.forEach(u => {
  sql += `UPDATE apsol.empresas SET provincia = ${u.provincia ? `'${u.provincia}'` : 'NULL'}, pais = '${u.pais}', industria = ${u.industria ? `'${u.industria}'` : 'NULL'}, tamanio = ${u.tamanio}, dias_espera_facturacion = ${u.dias_espera} WHERE id = '${u.id}';\n`;
});

sql += '\n-- Inserts (New Companies)\n';
inserts.forEach(i => {
  sql += `INSERT INTO apsol.empresas (nombre, provincia, pais, industria, tamanio, dias_espera_facturacion) VALUES ('${i.nombre}', ${i.provincia ? `'${i.provincia}'` : 'NULL'}, '${i.pais}', ${i.industria ? `'${i.industria}'` : 'NULL'}, ${i.tamanio}, ${i.dias_espera});\n`;
});
...
if (companyIdsToRefresh.length > 0) {
  ...
  reasons.forEach(r => {
    sql += `INSERT INTO apsol.razones_sociales (empresa_id, razon_social, cuit) VALUES ('${r.empresa_id}', '${r.razon_social}', ${r.cuit ? `'${r.cuit}'` : 'NULL'});\n`;
  });
}
